...
/**
 * @Author szc
 * @create 2022/7/5 22:31
 */
@RestController
@RequestMapping("/Inf/TargetExtraction")
public class TargetExtractionController {
    ...
    public ResponseResult filesuploadAndRun(@RequestParam("files") MultipartFile[] files){
        return uploadAndRun.goFiles(rabbitTemplate, fileManageService, files, TaskId.TARGET_EXTRACTION);
    }

    @PostMapping("/uploadAndRun/zip")
    @ResponseBody
    public ResponseResult zipUploadAndRun(@RequestParam("zip") MultipartFile zip){
        return uploadAndRun.goZip(rabbitTemplate, fileManageService, zip, TaskId.TARGET_EXTRACTION);
    }
